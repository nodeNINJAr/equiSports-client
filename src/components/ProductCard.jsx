...
const ProductCard = ({ product, uniqueProduct }) => {
  const {
    _id,
    productUrl,
    productRating,
        ...
        </div>
        {
          uniqueProduct &&  <>
            ...
            <div className="absolute bottom-0 right-0  w-0 bg-[#F44A16] hover:bg-[#ff3932] text-white text-center opacity-0 group-hover:w-24 group-hover:opacity-100 transition-all duration-300 ease-in-out rounded-full">
              ...
              <Link to={`/view-details/${_id}`}>
                {" "}
                <button className="w-full h-full text-white transition-all ease-in-out duration-200 hover:text-black font-medium font-barlow tracking-widest text-lg flex justify-center items-center gap-1 ">
